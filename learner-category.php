...
<?php
try {
    $sql = "INSERT INTO tbllearnerCategory (ExamName, Session, LearnerCategory, PassStatus, StudentCount)
    SELECT 
        tblexams.ExamName,
        tbllearner.Session,
        tbllearner.LearnerCategory,
        tbllearner.PassStatus,
        COUNT(DISTINCT tbllearner.StudentId) AS StudentCount
    FROM tbllearner
    JOIN tblexams ON tblexams.id = tbllearner.ExamTypeId
    GROUP BY tbllearner.LearnerCategory, tbllearner.PassStatus, tblexams.ExamName, tbllearner.Session";
...
$fetchSql = "SELECT * FROM tbllearnerCategory";
...
if ($query->rowCount() > 0) {
    ...
    <tr>
        ...
        <td><?php echo htmlentities($result->LearnerCategory); ?></td>
        <td><?php echo htmlentities($result->PassStatus); ?></td>
        <td><?php echo htmlentities($result->StudentCount); ?></td>
        <td>
