...
<div class="form-group">
<label for="examtype" class="col-sm-2 control-label">Select Exam Type</label>
<select name="examtype" class="form-control" id="examtype" required="required">
<option value="">Select Exam Type</option>
<?php 
$sql = "SELECT id, ExamName FROM tblexams";
$query = $dbh->prepare($sql);
$query->execute();
$exams = $query->fetchAll(PDO::FETCH_OBJ);
if($query->rowCount() > 0) {
    foreach($exams as $exam) { ?>
        <option value="<?php echo htmlentities($exam->id); ?>"><?php echo htmlentities($exam->ExamName); ?></option>
<?php }} ?>
</select>
</div>
