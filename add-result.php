...
$examtype = $_POST['examtype']; // Get the exam type
...
$mar = $mark[$i];
$sid = $sid1[$i];
$sql = "INSERT INTO tblresult(StudentId, ClassId, SubjectId, marks, ExamTypeId, Session) 
VALUES(:studentid, :class, :sid, :marks, :examtype, :session)";
...
var clid = $(".clid").val();
var val = $(".stid").val();
var examtype = $("#examtype").val(); // Fetch the exam type
var abh = clid + '$' + val;
...
data: {
studclass: abh,
examtype: examtype // Send the exam type with the request
},
success: function(data) {
$("#reslt").html(data);
}
...
<div class="col-sm-10">
<select name="examtype" class="form-control" id="examtype" required="required" onChange="getresult();>
<option value="">Select Exam Type</option>
<?php 
$sql = "SELECT * FROM tblexams";
$query = $dbh->prepare($sql);
$query->execute();
$results = $query->fetchAll(PDO::FETCH_OBJ);
if($query->rowCount() > 0) {
...
<?php echo htmlentities($result->ExamName); ?>
</option>
<?php } 
} ?>
</select>
